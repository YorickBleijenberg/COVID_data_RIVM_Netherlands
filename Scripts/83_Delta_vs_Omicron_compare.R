...
year.compare.cases$DmJDays <- year.compare.cases$dateInTable -  as.Date("2021-11-10")
year.compare.cases$onefiveDays <- year.compare.cases$dateInTable -  as.Date("2021-12-28")




rivm_file <- paste0("rivm.file.compare_D_O.csv")
write.csv2(year.compare.cases, rivm_file, row.names=FALSE) 


####  percentage calculation cases  ####
then.minus.two.weeks = (today-(48))
...
ggplot(year.compare.cases)+
  ...
  scale_color_manual(values = c("#3c81b9","#e5292b"),labels=c ("Delta", "Omicron") )+
  
  scale_x_continuous(limits = c(-21, 50), breaks = c(-21,-14,-7,0,7,14,21,28,35,42,49,56,63,70))+
...
LCPS.compare$DmJDays <- LCPS.compare$Datum -  as.Date("2021-11-10")
LCPS.compare$onefiveDays <- LCPS.compare$Datum -  as.Date("2021-12-28")
...
NICE.compare$DmJDays <- NICE.compare$date -  as.Date("2021-11-10")
NICE.compare$onefiveDays <- NICE.compare$date -  as.Date("2021-12-28")
