...
bla <- bla[bla$week>28&bla$week<=39,]
...
geom_tile(size=1.5,color="#F5F5F5")+
  ...
  labs(title = "Geconstateerde besmettingen COVID-19",
       subtitle = "Aantal positief geteste mensen per 100.000 binnen de leeftijdsgroep. Week 39 zal nog sterk stijgen",fill=NULL,
...
ggsave("data/02_leeftijd_heatmap.png",width=10, height = 8)




#Gewenste weken subsetten
bla <- bla[bla$week>31&bla$week<=39,]
...
ggplot(bla,aes(Agegroup,phd,fill=week))+
  ...
  labs(title = "Geconstateerde besmettingen COVID-19",
       subtitle = "Aantal positief geteste mensen per 100.000 binnen de leeftijdsgroep. Week 39 zal nog sterk stijgen. ",
       fill="Week",
       caption = paste("Bron data: RIVM / CBS ",Sys.Date()))+
