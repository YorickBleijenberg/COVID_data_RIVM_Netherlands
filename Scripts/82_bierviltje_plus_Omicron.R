...
ggplot()+
  ...
  labs(title = "Bierviltje - ziekenhuisopnames - Omicron",
       subtitle = "Dit klopt niet, want bierviltje.",
       caption = paste("Source: NICE / RIVM | Plot: @YorickB | ",Sys.Date()))+
  
  theme( plot.title = element_text(hjust = 0.5,size = 30,face = "bold"))

ggsave("data/plots/bierviltje_plus_2_omicron.png",width=16, height = 9)
...
zkh_new <- zkh_new[zkh_new$date>"2021-03-20"&zkh_new$date<=today-2,]
