...
lcps_working_1x  <- lcps_working_1[,c("date","IC_opname","clinic_opname")]
lcps_working_2x  <- lcps_working_2[,c("date","IC_opname","clinic_opname")]

bothdfs33 <- rbind(lcps_working_2x, lcps_working_1x)

#colnames(bothdfs33) <- c("date", "B.IC_opname", "A.clinic_opname")
#lcps_working_tot_c_1 <- lcps_working_1_short[,c("date","IC_covid_nl","clinic_nl")]
lcps_working_new_hosp_long <- gather(bothdfs33, "IC_opname","clinic_opname", -date)
colnames(lcps_working_new_hosp_long) <- c("date", "type", "number")

#lcps_working_new_hosp_long <- filter(lcps_working_new_hosp_long$date > "2020-10-14") 
...
bothdfs33 <-bothdfs33[order(bothdfs33$date),]
lcps.sh.3 <- tail(bothdfs33,n=2)
...
ggplot(data = lcps_working_new_hosp_long, mapping = aes(x = date, y = number, color = type, fill = type))+
  geom_bar(stat='identity')+
...
if (hosp.total.c < hosp.total.b) {
  hosp.total.dot <- intToUtf8(0x1F7E2)     ### groen
} else if (hosp.total.c > hosp.total.b) {
