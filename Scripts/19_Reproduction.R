...
persco.df=data.frame(date=as.Date(c("2020-09-19", "2020-09-29", "2020-10-14", "2020-11-04")), 
                        event=c("kroeg uurtje eerder dicht", "We gaan voor R=0,9","Semi-lockdown  (0.75-0.99)", "verzwaring semi-lockdown  (0.72-0.91)"))
   
r.estimate.df = data.frame(date_start=as.Date(c("2020-09-19", "2020-09-29", "2020-10-14", "2020-11-04")), 
                           date_end=as.Date(c("2020-10-19", "2020-10-29", "2020-11-04", "2020-11-05"))
                           ) 
...
ggplot(last.date.old.wide, aes(x=Date, y=Rt_avg, group = 1))+
 ...
 # geom_line(aes(y = Rt_low), lwd=0.6) +
  ...
  #geom_ribbon(aes(ymin=Rt_low,ymax=Rt_up), fill="darkred", alpha = 0.4) +
  geom_line(aes(y = Rt_avg, color = "Effectieve R"), color = "black",lwd=2) +
  
  scale_x_date(limits=as.Date(c("2020-9-1", NA)), date_breaks = "1 week") +
  ...
  theme(axis.title.x=element_blank(),
    ...
    labs(x = "Datum",
       y = "Reproductiegetal",
       color = "Legend") +
    ggtitle("Reproductiegetal, model 3 november") + 
  geom_hline(yintercept=1)+

  annotate("rect", xmin = as.Date("2020-09-29"), xmax = as.Date("2020-11-01"), ymin = 0.95, ymax = 0.85, fill = "red", alpha = 0.3)+
  annotate("rect", xmin = as.Date("2020-10-14"), xmax = as.Date("2020-11-04"), ymin = 0.75, ymax = 0.99, fill = "blue", alpha = 0.2)+
  annotate("rect", xmin = as.Date("2020-11-04"), xmax = as.Date("2020-11-15"), ymin = 0.72, ymax = 0.91, fill = "green", alpha = 0.3)+
...
ggplot(last.date.old.wide, aes(x=Date, y=Rt_avg))+
  ...
  scale_x_date(date_breaks = "1 week",
               ...
               limits = as.Date(c("2020-08-01", "2020-11-01")))+
