...
dat <- fromJSON(txt = "https://data.rivm.nl/covid-19/COVID-19_uitgevoerde_testen.json")
tested_daily <- as.data.frame(dat)
tested_daily$date <- as.Date(tested_daily$Date_of_statistics)

tested_daily.toedit <-tested_daily
tested_daily.toedit <- tested_daily.toedit[ -c(1:5)]

  colnames(tested_daily.toedit) = c("tests", "cases", "date")
# colnames(tested_daily.toedit) = c("cases", "perc_pos", "date")

 ### tested_daily.toedit <- aggregate(tested_daily.toedit$cases,     by=list(date=copy.aantal.gem.dag$date), FUN=sum)

  tested_daily.toedit <- (tested_daily.toedit  %>% filter(date > "2020-12-01"))
...
tested_daily.toedit$perc_pos = tested_daily.toedit$cases / tested_daily.toedit$tests*100
tested_daily.toedit$MA_perc  <- round(frollmean(tested_daily.toedit$perc_pos,7),1)
...
ggplot(tested_daily.toedit, aes(MA_perc, vdphd, colour=date))+
  
  annotate("rect", xmin = 0, xmax =4, ymin = 500, ymax = 725, color = "black",fill = "darkred", alpha = 0.9)+
  annotate("rect", xmin = 4, xmax =22, ymin =500, ymax = 725, color = "black",fill = "darkred", alpha = 0.9)+
  
  annotate("rect", xmin = 0, xmax =4, ymin = 150, ymax = 500, color = "black",fill = "red", alpha = 0.5)+
  annotate("rect", xmin = 4, xmax =22, ymin =150, ymax = 500, color = "black",fill = "red", alpha = 0.5)+
  
  annotate("rect", xmin = 0, xmax =4, ymin = 50, ymax = 150, color = "black",fill = "orange", alpha = 0.9)+
  annotate("rect", xmin = 4, xmax =22, ymin = 50, ymax = 150, color = "black",fill = "red", alpha = 0.5)+
  
  annotate("rect", xmin = 0, xmax =4, ymin = 25, ymax = 50, color = "black", fill = "orange", alpha = 0.9)+
  ...
  annotate("rect", xmin = 4, xmax = 22, ymin = 0, ymax = 25,color = "black", fill = "orange", alpha = 0.9)+ 
  ...
  scale_x_continuous(labels = scales::percent_format(scale = 1,accuracy = 1), breaks = c(0,1,4,5,10,15,20))+
  scale_color_gradient(low="gray", high="darkblue")+
  
  scale_y_continuous(limits = c(0, 725),breaks = c(0,25,50,75,150,500))+
...
animation.thing <- ggplot(tested_daily.toedit, aes(perc_pos, vdphd, colour=date))+
 ...
 geom_point(   size =3)+
  ...
  labs(title = "ECDC check",
       subtitle = "Nieuwe gevallen per 100.000, per 14 dagen  VS  percentage positieve testen \n -kleurcodering volgens ECDC richtlijn-",
       caption = paste("Bron: ECDC / RIVM | Plot: @YorickB | ",Sys.Date()))+
