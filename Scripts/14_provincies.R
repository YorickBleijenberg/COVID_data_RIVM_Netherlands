...
ggplot(data = number_new_prov_short, ) + 
  ...
  # geom_bar(stat='identity', mapping = aes(x = Date, y = newCases), fill = "#8FAADC")+
...
ggplot(data = number_new_prov_short, mapping = aes(x = Date, y = MAnewCases, color = Province)) + 
  geom_line(size = 2)+
  theme_classic()+
  xlab("")+ 
  ylab("")+
...
ggsave("data/18_Province_newCasesMA.png",width=16, height = 9)
