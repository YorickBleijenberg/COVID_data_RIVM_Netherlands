...
colnames(df.daling.4) = c("Datum", "r7", "r8","r9", "r10", "r12", "r14", "r16", "r21", "r28")
...
#kerst.1.df=data.frame(date=as.Date(c("2020-12-25", "2020-12-05")),event=c(kerst,sinter))
kerst.1.df=data.frame(date=as.Date(c("2020-12-25")),event=c(kerst))
...
ggplot(Merged_data_short)+
 ...
 # geom_point(stat='identity', mapping = aes(x=fixedDate, y=cases, fill = "x"))+
  ...
  annotate("text", x = as.Date("2020-07-05"), y = 500,  label = "Waakzaam", size=10,color = "black",face = "bold", hjust ="left")+
 ...
 # geom_vline(xintercept = as.Date("2020-12-25"), linetype = "dashed", size = 1.5, color = "darkgreen")+
  ...
  geom_vline(data=kerst.1.df,  mapping=aes(xintercept=date), linetype = "dashed", size = 1.5, color = "darkgreen")+
 geom_text(data=kerst.1.df  , mapping=aes(x=date, y=6500, label=event), size=8, angle=90, vjust=-0.4, hjust=0)+
