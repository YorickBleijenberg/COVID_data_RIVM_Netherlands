...
vacc.effect.age.ICU <- (vacc.effect.age  %>% filter(Type == type.of.data & Datum > "2020-12-01")) # & Datum < "2021-02-01"))  #  Klinisch
...
maxClinyoung <-  1        # max(vacc.effect.age.ICU$MA.young.Change, na.rm = TRUE)
maxClinThree <-  1.285714 # max(vacc.effect.age.ICU$MA.three.Change, na.rm = TRUE)
maxClinFour  <-  3.714286 # max(vacc.effect.age.ICU$MA.four.Change, na.rm = TRUE)
maxClinFive  <-  13.71429 # max(vacc.effect.age.ICU$MA.fifty.Change, na.rm = TRUE)
maxClinSix   <-  18.85714 # max(vacc.effect.age.ICU$MA.sixty.Change, na.rm = TRUE)
maxClinSeven <-  19.28571 # max(vacc.effect.age.ICU$MA.seven.Change, na.rm = TRUE)
maxClin80    <-  2.571429 # max(vacc.effect.age.ICU$MA.old80.Change, na.rm = TRUE)  # 2.1428571 - 3.142857 / 3.2857143
...
ggplot(relative.table.clinic.long, aes(x=Datum, y= value, color = factor(key, levels=c("MA.three.Change.relative",
 ...
 #  scale_color_brewer(palette = "RdYlBu", labels=c("50-", "50-59","60-69", "70-79","80+" ))+  
  
  scale_color_manual( values=c("#c49a00","#53b400","#00c094","#00b6eb","#a58aff"), labels=c("30-39", "40-49","50-59","60-69","70-79"))+
...
ggplot(abs.table.clinic.long, aes(x=Datum, y= value, color = factor(key, levels=c("MA.young.Change",
 ...
 limits = as.Date(c("2021-06-10", NA)))+
  ...
  #scale_color_brewer(palette = "PuBu" , labels=c("20-","20-29","30-39", "40-49","50-59","60-69","70-79","80+" ))+  # 
